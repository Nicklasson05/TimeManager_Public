...
func apiRequest(selectEntry *widget.SelectEntry, apiKey string, projectID string) {
	...
	for _, line := range pritty {
		writeToTasks(line, Directory+"/response.txt")
	}
	...
	for _, line := range pritty0 {
		writeToTasks(line, Directory+"/responseIID.txt")
	}
	...
	for _, line := range pritty1 {
		writeToTasks(line, Directory+"/responsePJID.txt")
	}
	FilterRespons("title", "project_id", Directory+"/responsePJID.txt", Directory+"/issuePJIDHolder.txt")
}
...
func readFromTasks(Contains string, selectEntry *widget.SelectEntry) {
	...
	TEMP9 := ""
	data := []byte(TEMP9)
	gerr := os.WriteFile(Directory+"/Tasks.txt", data, 0644)
	if gerr != nil {
		fmt.Println("Error while writeing to Tasks")
	}
	...
	for Scanner.Scan() {
		...
		if Clear {
			...
			//fmt.Printf(line + "\n")
			writeToTasks(line, Directory+"/Tasks.txt")
			Clear = false
		}
	}
	// Updating selectEntry options
	var selectEntryList []string
	file, ferr := os.Open(Directory + "/Tasks.txt")
	if ferr != nil {
		panic(ferr)
	}
	scanner := bufio.NewScanner(file)
	...
	TEMP90 := ""
	data0 := []byte(TEMP90)
	qerr := os.WriteFile(Directory+"/response.txt", data0, 0644)
	if qerr != nil {
		fmt.Println("Error while writeing to Tasks")
	}
}
...
func filterRequwstForIID(title string, iid string) {
	...
	gerr := os.WriteFile(Directory+"/issueIDHolder.txt", data, 0644)
	if gerr != nil {
		fmt.Println("Error while writeing to Tasks")
	}

	// Write Tasks to Tasks.txt
	File, err := os.Open(Directory + "/responseIID.txt")
	if err != nil {
		panic(err)
	}
	...
	for Scanner.Scan() {
		...
		if Clear0 {
			line = line[0:]
			line = line[:]
			writeToTasks(line, Directory+"/issueIDHolder.txt")
			Clear0 = false
		}
	...
	gerr0 := os.WriteFile(Directory+"/responseIID.txt", data0, 0644)
	if gerr0 != nil {
		fmt.Println("Error while writeing to Tasks")
	}
}
func getIIDFromTask(task string) string {
	...
	File, err := os.Open(Directory + "/issueIDHolder.txt")
	if err != nil {
		panic(err)
	}
...
func GetPJID(task string) string {
	var lineHolder string
	File, err := os.Open(Directory + "/issuePJIDHolder.txt")
	if err != nil {
		panic(err)
	}
