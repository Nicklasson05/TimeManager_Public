...
func MakeShowTime() {
	...
	ShowTimeRefresh = widget.NewButtonWithIcon("Refresh", theme.MediaReplayIcon(), func() {
		...
		if TimeLogs != nil {
			...
			ShowTimeEntry.Text = ""
			ShowTimeEntry.Text += "+---------------------+---------------------------+------------------------------------:\n"
			ShowTimeEntry.Text += "|  Total              |  Spent Time          | Task      \n"
			ShowTimeEntry.Text += "+===========-+-==============-+-====================:\n"
			ShowTimeEntry.Text += "\n"
			ShowTimeEntry.Text += "\n"
			ShowTimeEntry.Text += "+---------------------+---------------------------+------------------------------------:\n"
			ShowTimeEntry.Text += "|  Indivudual   |  Spent Time          |  Task                \n"
			ShowTimeEntry.Text += "+===========-+-==============-+-====================:\n"
		}
...
func ShowTimeLogs() {
	...
	for _, v := range TickerHolder {
		...
		if len(SpentTime) == 11 {
			infoLine = "|                          |     " + SpentTime + "       |  " + v.title + "\n"
		} else if len(SpentTime) == 10 {
			infoLine = "|                          |     " + SpentTime + "         |  " + v.title + "\n"
		} else if len(SpentTime) == 9 {
			infoLine = "|                          |     " + SpentTime + "           |  " + v.title + "\n"
		}
		ShowTimeEntry.Text += infoLine
		ShowTimeEntry.Text += "+---------------------+---------------------------+------------------------------------:\n"
	}
	...
	SpentTime := TranslateSeconds(int(totalTimeSpent))
	if len(SpentTime) == 12 {
		ShowTimeEntry.Text += "| " + SpentTime + " |                                  |   \n"
	} else if len(SpentTime) == 11 {
		ShowTimeEntry.Text += "| " + SpentTime + "   |                                  |   \n"
	} else if len(SpentTime) == 10 {
		ShowTimeEntry.Text += "|   " + SpentTime + "   |                                  |   \n"
	} else if len(SpentTime) == 9 {
	...
	ShowTimeEntry.Text += "|  Indivudual   |  Spent Time          |  Task                \n"
	ShowTimeEntry.Text += "+===========-+-==============-+-====================:\n"
	// 2 2 - |                          |     10 h 10 min       | Chanbannan
	// 2 1 - |                          |     10 h 0 min         | Chanbannan
	// 1 1 - |                          |     0 h 0 min           | Chanbannan
	for _, v := range TimeLogs {
		var infoLine string
